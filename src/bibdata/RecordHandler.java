...
public class RecordHandler extends DefaultHandler {
	...
	if (returnVal == JFileChooser.APPROVE_OPTION) {
	    ...
	    // Logger.getGlobal().log(Level.SEVERE, "Saving records to: " + recordfile.getAbsolutePath());
	...
	if (returnVal == JFileChooser.APPROVE_OPTION) {
	    ...
	    // This is where a real application would save the file.
	    // Logger.getGlobal().log(Level.SEVERE, "Saving records to: " + keywordfile.getAbsolutePath());
	} else {
	    Logger.getGlobal().log(Level.SEVERE, "Cannot save file");
	    return;
	}
	...
	if (returnVal == JFileChooser.APPROVE_OPTION) {
	    ...
	    // This is where a real application would save the file.
	    // Logger.getGlobal().log(Level.SEVERE, "Saving authors to: " + authorfile.getAbsolutePath());
	} else {
	    Logger.getGlobal().log(Level.SEVERE, "Cannot save file");
	    return;
	}
	...
	try {
	    ...
	    Logger.getGlobal().log(Level.SEVERE, "Could not write headers");
	}
    }
	...
	try {
	    recordbf.close();
            Logger.getGlobal().log(Level.INFO, "Succesfully finished writing to " + recordfile.getName());
	} catch (IOException ex) {
	...
	if (localName == "BibDocumentsGent.Record") {
		...
		Logger.getGlobal().log(Level.SEVERE, "Error writing to " + recordfile.getName());
	    }
	    record.clear();
	}
    }
	...
	if (parsemode == PARSEMODE_RECORD && parsekey != "NONE") {
		...
		default:
		    // Logger.getGlobal().log(Level.SEVERE, "parse key" + parsekey);
